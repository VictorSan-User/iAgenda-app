...
function openModal(date) {
    ...
    if (eventDay) {
        document.getElementById('eventText').innerText = `Título: ${eventDay.title}\nInício: ${eventDay.startTime}\nFim: ${eventDay.endTime}`;
...
function load() {
    ...
    for (let i = 1; i <= paddinDays + daysMonth; i++) {
        ...
        if (i > paddinDays) {
            ...
            const eventDays = events.filter(event => event.date === dayString);
...
function saveEvent() {
    ...
    if (title && startTime && endTime) {
        ...
        alert('Tarefa criada com sucesso!'); // Alerta ao criar a tarefa
        closeModal();
    } else {
...
function deleteEvent() {
    ...
    localStorage.setItem('events', JSON.stringify(events));
    alert('Tarefa excluída com sucesso!'); // Alerta ao excluir a tarefa
    closeModal();
}
